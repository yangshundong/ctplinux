...
void CTraderSpi::OnRspOrderInsert(CThostFtdcInputOrderField* pInputOrder, CThostFtdcRspInfoField* pRspInfo, int nRequestID, bool bIsLast)
{
	...
	if (pRspInfo)
	{
		LOG("\t<OnRspOrderInsert> ErrorMsg [%s]\n", pRspInfo->ErrorMsg);
...
void CTraderSpi::OnRtnOrder(CThostFtdcOrderField* pOrder)
{
	...
	if (pOrder)
	{
		...
		LOG("<OnRtnOrder>  \tStatusMsg [%s]\tsystem_time\t %d \n", pOrder->StatusMsg, millisec_since_epoch % 100000000);
...
{
	if (strcmp(pTrade->InstrumentID, g_chInstrumentID) != 0)return;
	if (pTrade->Direction== THOST_FTDC_D_Buy)
	{
		posflag ++;
		if (pTrade->OffsetFlag == THOST_FTDC_OF_Open)
		{
			open_price_shfe = pTrade->Price;
			tp();
	...
	if (pTrade)
	{
		...
		LOG("³É½» \tPrice [%.8lf]\tsystem_time\t %d\n", pTrade->Price, millisec_since_epoch % 100000000);
	...
	//	/*auto_position += pTrade->Volume;
	//	open_price = pTrade->Price;*/
	//}
...
void CTraderSpi::OnErrRtnOrderInsert(CThostFtdcInputOrderField* pInputOrder, CThostFtdcRspInfoField* pRspInfo)
{
	...
	if (pRspInfo)
	{
		LOG("<OnErrRtnOrderInsert>\tErrorMsg [%s]\n", pRspInfo->ErrorMsg);
