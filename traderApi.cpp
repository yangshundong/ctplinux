...
{
	int i = m_pApi->ReqOrderInsert(pInputOrder, nRequestID);
	
	if (pInputOrder)
	{
		string s1 = pInputOrder->Direction == THOST_FTDC_D_Buy ? "买" : "卖";
		string s2 = pInputOrder->CombOffsetFlag[0] == THOST_FTDC_OF_Open ? "开" : "平";
		...
		LOG(" % s % s\t Price[% .8lf] system__time \t % d \n", s1, s2 ,pInputOrder->LimitPrice,millisec_since_epoch % 100000000)
