...
//
//
//void send_order(int direction_flag, int open_flag);
...
//class CSimpleMdHandler;
//
//
//CSimpleHandler* global_tdapi;//全局的tdapi  
//CSimpleMdHandler* global_mdapi;//全局的mdapi  
//
////std::map<int, bool> order_id_map;//用于防止成交后反复推送
//using namespace std;
//int posflag = 0;//自动计算的仓位  下单了盲算
//int auto_position = 0;//暂时不准备用了
//double open_price_shfe=0;
//double last_ask = 0;
//double last_bid = 0;
//double price_buffer[20];
//int order_id = 0;
//double g_chprice_tick;//最小价格变动单位
//int g_chinstrumentsize;
//int g_chover_price;//规定下单的超价
//int g_chjump_tick_now;
//int g_chjump_tick_before;
//double g_chtakeprofit;//止盈
//double  g_chstoploss;//止损  单位人民币
//
//double g_chbig_trigger;//触发
//double g_chsmall_trigger;
//double g_chsix_trigger;
//
//long last_ticktime=0;//记录上一次来tick的毫秒数  毫秒数要改正
//long last_opentime=0;//记录上一次开仓的毫秒数  
//
//int g_chholdtime_ms = 0;//持仓最短时间
//int g_chlongest_hold_ms = 0;//持仓最长时间
//
//HANDLE hMapFile;
////const char* buffername;//ShunHe_Connection_000
//double* sharedVariable;
//double memo_buffer[4];
//
//HANDLE hMutex_a;
//HANDLE hMutex_b;
//
...
//class CSimpleMdHandler;
//
////CSimpleHandler* global_sh;
////CSimpleMdHandler* global_ash;
//
//
//
//
...
//		chrono::duration_cast<chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch())
//		.count() % 100000000;
//	int ret = millisec_since_epoch;
//	return ret;
//}
//
////交易类
...
//		SetEvent(g_hEvent);
//		
//		cout << "system_time\t" << get_milli_second() % 1000000 << endl;
//
//
//
//
...
//		
//		cout << "system_time\t" << get_milli_second() << endl;
//		printf("\t客户端认证 = [%d]\n", b);
//
//
//	}
...
//	{
//		if (strcmp(pInstrument->InstrumentID, g_chInstrumentID) != 0)
//		{
//			return;
//		}
...
//		}
//
//		g_chprice_tick = pInstrument->PriceTick;
//		g_chinstrumentsize = pInstrument->VolumeMultiple;
//		g_chstoploss = g_chprice_tick * g_chinstrumentsize * atof(getConfig("config", "stoploss").c_str());
//		g_chtakeprofit = g_chprice_tick * g_chinstrumentsize * atof(getConfig("config", "takeprofit").c_str());
//	}
//
//	//请求查询投资者结算结果
...
//	{
//		if (strcmp(pOrder->InstrumentID, g_chInstrumentID) != 0)return;
//
//		
//		
//		{
...
//		LOG("ontick");
//		return;
//		if (strcmp(pDepthMarketData->InstrumentID, g_chInstrumentID) != 0)return;
//		//if (pDepthMarketData->AskVolume1 == 0 || pDepthMarketData->ClosePrice == 0)return;错误
//		if (pDepthMarketData->AskVolume1 == 0 || pDepthMarketData->BidVolume1 == 0)return;
//
//
//		last_ask = pDepthMarketData->AskPrice1;
//		last_bid = pDepthMarketData->BidPrice1;
//
//
//		int askvol = pDepthMarketData->AskVolume1 * 5 + pDepthMarketData->AskVolume2 * 4 + pDepthMarketData->AskVolume3 * 3 + pDepthMarketData->AskVolume4 * 2 + pDepthMarketData->AskVolume5;
//		int bidvol = pDepthMarketData->BidVolume1 * 5 + pDepthMarketData->BidVolume2 * 4 + pDepthMarketData->BidVolume3 * 3 + pDepthMarketData->BidVolume4 * 2 + pDepthMarketData->BidVolume5;
//
//		for (int i = 0; i < 2; i++) {
//			price[i] = price[i + 1];
//		}
//		price[2] = 1.0 * (pDepthMarketData->AskPrice1 + pDepthMarketData->BidPrice1) / 2;
//		
//		
//		double predict_price = 1.0 * (bidvol * pDepthMarketData->AskPrice1 + askvol * pDepthMarketData->BidPrice1) / (askvol + bidvol)
//			+ 1.0 * (bidvol - askvol) / (askvol + bidvol) * (pDepthMarketData->AskPrice1 - pDepthMarketData->BidPrice1);
//		
//		last_ticktime = get_milli_second();
//
//		if (last_ticktime -last_opentime < g_chholdtime_ms)
//		{
//			return;
//		} 
//		double profit = 0;
//		if (posflag > 0)
//		{
//			profit = posflag * (last_bid - open_price_shfe)* g_chinstrumentsize;
//		}
//		else if (posflag < 0)
//		{
//			profit = posflag * (last_ask - open_price_shfe)* g_chinstrumentsize;
//		}
//		
//
//		if (posflag != 0 && (profit<g_chstoploss || profit > g_chtakeprofit))
//		{
//			if (profit < g_chstoploss)
//			{
//				LOG("止损了 profit:%f  sl: %f \n", profit, g_chstoploss);
//			}
//			if (profit > g_chtakeprofit)
//			{
//				LOG("止盈了 profit:%f  tp: %f\n", profit, g_chtakeprofit);
//			}
//			
//			addTask(
//				[]() {
